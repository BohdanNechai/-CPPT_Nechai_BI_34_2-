import java.io.*;
import java.util.Scanner;

import static java.lang.System.out;

/**
 *
 * y=tg(x)/(sin(4x) – 2cos(x)) це мій вираз
 *
 */

public class CalculateExpression {
    /**
     *
     * @param args args
     * @throws FileNotFoundException if file not found
     */

    public static void main(String[] args) throws FileNotFoundException {
        try
        {

            Scanner in = new Scanner(System.in);
            PrintWriter fout = new PrintWriter(new File("result"));
            try
            {
                try
                {
                    WriteAndRead war = new WriteAndRead();
                    Equations eq = new Equations();
                    out.print("Enter X: ");
                    int input = in.nextInt();
                    out.println(Equations.calculate(input));

                    war.createFile("lab_02");
                    war.WriteToFile("lab_02",Equations.calculate(input));
                    out.println("read from file " + Equations.calculate(war.ReadFromFile("readFrom.txt")));
                }
                finally
                {
// Цей блок виконається за будь-яких обставин
                    fout.flush();
                    fout.close();
                }
            }
            catch (CalcException ex)
            {
// Блок перехоплює помилки обчислень виразу
                out.print(ex.getMessage());
            }
        }
        catch (FileNotFoundException ex)
        {
// Блок перехоплює помилки роботи з файлом навіть якщо вони // виникли у блоці finally
            out.print("Exception reason: Perhaps wrong file path");
        }
    }
}


class CalcException extends ArithmeticException
{
    public CalcException(){}
    public CalcException(String cause)
    {
        super(cause);
    }
}

class Equations{
    /**
     *
     * @param x parameter
     * @return return res
     * @throws CalcException throws exception
     *
     */
    public static double calculate(int x) throws CalcException{
        double y,rad;
        rad = x * Math.PI / 180.0;
        try
        {
            y = (Math.tan(rad)/(Math.sin(4*x)) - 2*Math.cos(x));
            if (y==Double.NaN || y==Double.NEGATIVE_INFINITY || y==Double.POSITIVE_INFINITY || x==90 || x== -90)
                throw new ArithmeticException();
        }
        catch (ArithmeticException ex)
        {
// створимо виключення вищого рівня з поясненням причини виникнення помилки
            if (rad==Math.PI/2.0 || rad==-Math.PI/2.0)
                throw new CalcException("Exception reason: Illegal value of X for tangent calculation");
            else if (x==0)
                throw new CalcException("Exception reason: X = 0");
            else
                throw new CalcException("Unknown reason of the exception during exception calculation");
        }

        return y;
    }
}

class WriteAndRead{
        public String createFile(String pathName){
            try{
                File myObj = new File(pathName);
                if(myObj.createNewFile()){
                    out.println("Файл створено: " + myObj.getName());
                }else {
                    out.println("Файл вже існує.");
                }
                return myObj.getName();
            }catch (IOException e){
                out.println("Виникла помилка");
                e.printStackTrace();
            }
            return pathName;
        }
        public void WriteToFile(String pathName,double x){
            try{
                FileWriter fileWriter = new FileWriter(createFile(pathName));
                fileWriter.write("Результат рівняння: " + x);
                fileWriter.close();
                out.println("Успішно записано в файл");
            } catch (IOException e) {
                out.println("Виникла помилка");
                e.printStackTrace();
            }
        }
        public int ReadFromFile(String pathName){
            File myObj = new File(pathName);
            int res = 0;
            String data = "";
            try {
                Scanner myReader = new Scanner(myObj);
                while (myReader.hasNextLine()){
                    data = myReader.nextLine();
                    out.println(data);
                }
                res = Integer.parseInt(data);
                myReader.close();
            } catch (FileNotFoundException e) {
                out.println("Виникла помилка");
                e.printStackTrace();
            }
            return res;
        }
}


